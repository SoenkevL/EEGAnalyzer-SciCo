...
def select_metrics(name):
    ...
    """
    if name == 'avalanche_criticality':  ################################################################################
        metrics_name_list = []
        metrics_functions = []
        kwargs_list = []
        return metrics_functions, metrics_name_list, kwargs_list
    if name == '0-1_chaos':  #############################################################################################
        # we use denoising with the default parameters of the paper which is omitting the step
        metrics_name_list = ['K']
        metrics_functions = [eop.chaos.chaos_pipeline]
        kwargs_list = []
        return metrics_functions, metrics_name_list, kwargs_list
